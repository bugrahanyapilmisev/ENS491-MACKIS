"""
rebuild_kg_index_pattern.py - Rebuild the KG embedding index from pattern-validated files.

This script rebuilds kg_index.pkl to match the pattern-validated facts/triples.

Usage:
    python rebuild_kg_index_pattern.py
"""
...
# Use pattern-validated files
KG_FACTS_PATH = os.path.join(KG_DIR, "kg_facts_cleaned.json")
KG_TRIPLES_PATH = os.path.join(KG_DIR, "kg_triples_cleaned.json")
KG_INDEX_PATH = os.path.join(KG_DIR, "kg_index_cleaned.pkl")
...
def rebuild_index():
    """Rebuild the KG index from pattern-validated files."""
    
    print("=" * 60)
    print("[REBUILD KG INDEX] From pattern-validated files")
    print("=" * 60)
    ...
    print(f"[OK] Loaded {len(topics)} topics from pattern-validated facts")
